...
class Coords:
    ...
    def collided_right(co1, co2):
        if within_y(cop1, co2):
            if co1.x2 >= co2.x1 and co1.x2 <= co2.y1:
                return True
            return False

    def collided_top(co1, co2):
        if within_x(co1, co2):
            if c01.y1 <= co2.y2 and co1.y1 >= co2.y1:
                return True
            return False
    ...
    def animate(self):
        ...
        if self.x <0:
            if self.y != 0:
                ...
                self.game.canvas.itemconfig(self.image, \
                        image=self.images_left[self.ciurrent_image])
        elif self.x > 0:
                   if self.y !=0:
                    self.game.canvas.itemconfig(self.image, \
